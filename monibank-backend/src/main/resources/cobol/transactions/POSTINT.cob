      ...
      * 00500 BASIS POINTS MEANS 5.00 PERCENT PER YEAR.
       ...
       01  WS-LEGACY-TXN-KEY.
           05 WS-LEGACY-ACCOUNT-ID PIC X(13).
           05 WS-LEGACY-TXN-ID.
              10 WS-LEGACY-PREFIX  PIC X.
              10 WS-LEGACY-DATE    PIC X(8).
              10 WS-LEGACY-SUFFIX  PIC X(4).
      ...
      * PHYSICAL MBANK.ACCT RECORD - EXACTLY 119 BYTES.
       01  ACCOUNT-RECORD.
           ...
           05 ACCOUNT-ID-PARTS REDEFINES ACCOUNT-ID.
              10 FILLER             PIC X(9).
              10 ACCOUNT-ID-SUFFIX  PIC X(4).
      ...
      * MIGRATION GUARD FOR INTEREST RECORDS WRITTEN BY THE OLD VERSION.
      * THE OLD PROGRAM USED TYYYYMMDD0000 FOR EVERY ACCOUNT.  CHECK THE
      * CHECK THE COMPLETE PRIMARY KEY (ACCOUNT ID + OLD TXN ID)
      * BEFORE POSTING INTEREST FOR THIS ACCOUNT AND BUSINESS DATE.
           MOVE ACCOUNT-ID TO WS-LEGACY-ACCOUNT-ID.
           MOVE 'T' TO WS-LEGACY-PREFIX.
           MOVE REQUEST-BUSINESS-DATE TO WS-LEGACY-DATE.
           MOVE '0000' TO WS-LEGACY-SUFFIX.
      ...
      * NORMAL MEANS THE OLD VERSION ALREADY POSTED THIS ACCOUNT/DAY.
           IF WS-RESP = ZERO
               PERFORM RELEASE-ACCOUNT
                   THRU RELEASE-ACCOUNT-EXIT
               GO TO POST-ACCOUNT-INTEREST-EXIT.
      ...
      * DETERMINISTIC, ACCOUNT-SPECIFIC ID KEEPS RERUNS IDEMPOTENT AND
      * PREVENTS EVERY ACCOUNT FROM SHARING ONE NONUNIQUE AIX KEY.
           MOVE 'C' TO TXN-STATUS.
           MOVE ACCOUNT-ID TO TXN-ACCOUNT-ID.
           MOVE 'T' TO TXN-ID-PREFIX.
           MOVE REQUEST-BUSINESS-DATE TO TXN-ID-DATE.
           MOVE ACCOUNT-ID-SUFFIX TO TXN-ID-SUFFIX.
