...
import { ArrowDownAZ, Building2, CircleDollarSign, Eye, Plus, ReceiptText, RefreshCw, Search, ShieldCheck, ShieldX, Users, X } from 'lucide-react'
    ...
    <div className="mx-auto max-w-[1220px] space-y-4">
      ...
      <Panel>
        ...
        {accountsQuery.isError ? <LoadError noun="accounts" query={accountsQuery} /> : (
          <AccountsTable accounts={visibleAccounts} loading={accountsQuery.isLoading} onView={(account) => setDetailsId(account.accountId)} onStatement={(account) => navigate(`/statements?accountId=${encodeURIComponent(account.accountId)}`)} onStatus={(account) => { changeStatus.reset(); setStatusId(account.accountId) }} />
        )}
      ...
      <AccountDetailsModal account={details} onClose={() => setDetailsId(null)} onStatement={() => navigate(`/statements?accountId=${encodeURIComponent(details?.accountId || '')}`)} onStatus={() => { changeStatus.reset(); setDetailsId(null); setStatusId(details?.accountId || null) }} />
...
function AccountsTable({ accounts, loading, onView, onStatement, onStatus }) {
